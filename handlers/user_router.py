...
# исправленная версия функции, которая работает с базой данных
async def get_table_names_async(conn: AsyncConnection):
    # Используем run_sync для выполнения inspect на асинхронном соединении
...
async def cmd_start(message: Message):
    ...
    if not user_info:
        ...
        await message.answer(
            text='Привет! Давай начнем общаться. Для этого просто нажми на кнопку "Начать диалог"',
            reply_markup=start_kb()
        )
    else:
        # Если пользователь найден, очищаем диалог
